...
def crawler_link(url):
    ...
    photo_data = html.xpath('//ul/li/a')
    # 已经下载的图片张数
    nextLink = ''
    if len(html.xpath('//div[@class="page"]/a[last()]')) > 0 and html.xpath('//div[@class="page"]/a[last()]/text()')[0] == '下一页' :
        nextLink = html.xpath('//div[@class="page"]/a[last()]/attribute::href')[0]
    # 开始下载并保存5K分辨率壁纸
    ...
    # if nextLink != '':
    #     time.sleep(0.2)
    #     crawler_link('https://www.23jj.com'+nextLink)
    # else:
    save_data()
...
def save_data():
    data = open('./jjdatalist.txt', mode='w+')
...
def loop_picture():
    # global datalist
    data = open('./jjdatalist.txt', mode='r')
    data = data.read()
    datalist = data.split('\n')
    while len(datalist) > 0:
        ...
        for photo in copy_result:
            ...
            url = 'https://www.23jj.com'+photo['link']
            dir = photo['dir']
            #  创建一个文件夹存放我们下载的图片
            if not exists('./jjpic/' + str(dir)):
                makedirs('./jjpic/' + str(dir))
            # try:
            #     gevent.joinall([
...
def crawler_photo(url, dir):
    ...
    respond = get(url, headers=headers)
    html = etree.HTML(respond.text)
    file_url = html.xpath('//div[@id="content"]//img/attribute::src')
    if len(file_url) > 0 and file_url[0]:
        file_url = 'https:'+file_url[0]
        file_name_only = file_url.split('/')
        file_name_only = file_name_only[len(file_name_only) - 1]
        # 准备保存到本地的完整路径
        file_full_name = './jjpic/' + dir + '/' + file_name_only
        print('开始下载图片')

        # 开始下载图片da
        Down_load(file_url, file_full_name, 1, 1)
    nextLink = ''
    if len(html.xpath('//a[class="ch next"]/attribute::href')) > 0 and \
            html.xpath('//a[class="ch next"]/text()')[0] != '下一组»':
        nextLink = html.xpath('a[class="ch next"]/attribute::href')[0]
    if nextLink != '':
        time.sleep(0.5)
...
if __name__ == '__main__':
    ...
    if cf.has_option('str', 'list_finish'):
        ...
        gevent.joinall([
            gevent.spawn(crawler_link, 'https://www.23jj.com'),
        ])
    print('\n下载成功!')
